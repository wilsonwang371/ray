...
RayFunction BuildRayFunction(InvocationSpec &invocation) {
  ...
  } else if (invocation.remote_function_holder.lang_type == LangType::WASM) {
    auto function_descriptor = FunctionDescriptorBuilder::BuildWasm(
        invocation.remote_function_holder.function_name);
    return RayFunction(ray::Language::WASM, function_descriptor);
