...
impl RayWaContext {
    pub async fn init_ray_hostcalls(&mut self) -> Result<()> {
        // setup hostcalls
        match register_ray_hostcalls("sandbox", &self.runtime, &self.engine) {
            Ok(_) => {
                debug!("ray hostcalls registered");
                Ok(())
            }
            Err(e) => {
                error!("ray hostcalls registration failed: {}", e);
                Err(anyhow!("ray hostcalls registration failed: {}", e))
            }
        }
    }
}
...
impl RayWaContextFactory {
    ...
    async fn create_engine(
        ...
        let engine_type = match engine_type {
            ...
            WasmEngineTypeParam::Wavm => engine::WasmEngineType::WAVM,
            WasmEngineTypeParam::Wamr => engine::WasmEngineType::WAMR,
        };
...
async fn run_binary(args: &LauncherParameters) -> Result<()> {
    ...
    match ctx.engine.write() {
        ...
        Err(e) => {
            ...
            return Err(anyhow!("failed to acquire engine lock: {}", e));
        }
    }

    debug!("wasm module instantiated");

    ctx.init_ray_hostcalls().await?;
    ...
    match ctx.runtime.write() {
        Ok(mut rt) => rt.do_shutdown()?,
        Err(e) => {
            error!("failed to acquire runtime lock: {}", e);
            return Err(anyhow!("failed to acquire runtime lock: {}", e));
        }
    }
