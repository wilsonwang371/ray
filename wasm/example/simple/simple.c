...
float task_func(char a, short b, float c, float d, int e) {
  ...
  WAR_LOG(fmt_hex((unsigned char *)&res, sizeof(float)));
  return res;
}
...
int test_call() {
  ...
  // remote execution
  LOG_INFO("***remote execution start***");
  WAR_ASYNC_CALL(wbuf, task_func, 'A', 0x1234, 1.0f, 3.1415926f, 0x6789abcd);

  LOG_INFO("waiting for remote execution result");
  WAR_GET(wbuf, result_buf, &len);
...
int test_struct() {
  ...
  struct test_struct_type s = {1, 2.0f, 3.0, 4};
  LOG_INFO("***remote execution start***");
  WAR_ASYNC_CALL(wbuf, task_struct, WAR_STRUCT_VAR(s));
...
int test_bufptr() {
  unsigned char result_buf[100];
  unsigned int len = sizeof(result_buf);
  unsigned char param_buf[4096];

  warbuffer *wbuf;
  WAR_OBJID_ALLOC(wbuf);

  // initialize param_buf
  for (size_t i = 0; i < sizeof(param_buf); i++) {
    param_buf[i] = i % 256;
  }

  LOG_INFO("***remote execution start***");
  WAR_ASYNC_CALL(
      wbuf, task_buffer, WAR_BUFFER_VAR(param_buf, sizeof(param_buf)), sizeof(param_buf));
